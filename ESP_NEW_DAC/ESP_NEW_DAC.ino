...
void processFrame(uint16_t *words) {
  ...
  // Debug-Ausgabe jedes Frames während der Erkennung
  if (!detectionComplete) {
    DEBUG_SERIAL.print("[Erkennung] Frame: ");
  ...
  if (!detectionComplete && millis() - bootTime < DETECTION_DURATION_MS) {
    ...
    if (millis() - bootTime > DETECTION_DURATION_MS - 200) {
      ...
      if (pressWordIndex != -1 && tempWordIndex != -1) {
        ...
        DEBUG_SERIAL.println("❌ Fehler: Konnte Druck- oder Temperatur-Word nicht eindeutig erkennen.");
      }
    }
  ...
  if (!detectionComplete) return; // Noch nicht bereit
  ...
  // Druck: linear
  float pressBar = mapValue(rawPress, 0, 65535, 0.0, 10.0); // anpassen falls nötig
